...
router.post("/", async (req, res, next) => {
  try {
    ...
    let result = await reqWrapper(usr.createUser, [data]);
...
router.get("/", async (req, res, next) => {
  try {
    ...
    res.status(200).json({ resultData: user });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
});
...
router.post("/login", async (req, res, next) => {
  try {
    ...
    let result = await reqWrapper(usr.login, [
      req.body.data.email,
    ...
    res.status(result.status).json({
      resultData: {
        token: result.json.resultData.token,
        role: result.json.resultData.role,
      },
    });
  } catch (error) {
    res.status(400).json({ error: error.message });
